...
long jsonobj::AppendObj(const char *key, i64 val) {
    if(m_keycount>=m_tablesize){
        long err = ResizeTable(m_keycount ? m_keycount : 4);
        if(err<0) return err;
    }
...
i64 jsonobj::AppendObj(const char *key) { 
    i64 val; long err = jsonobj::Create(&val);
    if(!val) { return JSON_ERROR_OUTOFMEMORY; }
    err = AppendObj(key,val);
    if(err<0) {jsonobj::Delete(val); return err;}
    return val;
}
//append empty array
i64 jsonobj::AppendArray(const char *key) { 
    i64 val; long err = jsonarray::Create(&val);
    if(!val) { return JSON_ERROR_OUTOFMEMORY; }
    err = AppendObj(key,val);
    if(err<0) {jsonarray::Delete(val); return err;}
    return val;
}
i64 jsonobj::AppendNumber(const char *key, double num) {
    i64 number;
    long err = jsonnumber::Create(&number);
    if(err<0) { return err; }
...
i64 jsonobj::AppendBoolean(const char *key, bool p_b) {
    i64 boolean;
    long err = jsonboolean::Create(&boolean);
    if(err<0) { return err; }
